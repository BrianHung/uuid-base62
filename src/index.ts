 ...
 */
var cachedMappings = {}
function parseToDigitsArray(str: string, charset: string): number[] {

  let charsetMapping: Record<string, number> = cachedMappings[charset]
  if (charsetMapping === undefined) {
    charsetMapping = {}
    for (var i = 0; i < charset.length; i++) charsetMapping[charset[i]] = i
  ...
  for (var i = chars.length - 1; i >= 0; i--) {
    ...
    if (n === undefined) throw new RangeError(`String ${str} contains ${chars[i]} which is not part of the charset ${charset}.`)
    array.push(n)
  }
  return array
}
 ...
 */
export function convertBase(str: string, fromBase: string, toBase: string): string {
  var digits = parseToDigitsArray(str, fromBase)

  var outArray = []
  var power = [1]
  for (var i = 0; i < digits.length; i++) {
    if (digits[i]) outArray = add(outArray, multiplyByNumber(digits[i], power, toBase.length), toBase.length)
 ...
 */
export function uuidEncodeBase62(uuidString: string, base16 = base16charset, base62 = base62charset): string {
 ...
 * @return {string} uuid
 */
export function uuidDecodeBase62(baseString: string, base16 = base16charset, base62 = base62charset): string {
